...
def Get_fastq_files_total(bam_file,chr_start,chr_end,num_threads,Raw_fastqs_dir,xin):
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        for _num in range(len(bam_list)):
            ...
            pool.apply_async(Get_fastq_files_total,(bam_file,chr_start,chr_end,20,Raw_fastqs_dir_sample,"xin"))
            _num += 1
        pool.close()
        while len(active_children()) > 1:
            time.sleep(0.5)
