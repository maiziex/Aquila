...
def Extract_SV_info(hap_SV_file,output_file,chr_num):
    ...
    for key, val in SV_dict_contig_1.items():
        if homo_key_dict[key] == 1:
            ...
            if alt_1 == "-":  # del
                ...
                #if count_cov - 1 == sv_len_1:
                if float(count_cov - 1)/sv_len_1 >= 0.8:
            ...
            if alt_2 == "-":  # del
                if _step in range(start_2, end_2+1):
                    if _step in ref_dict_contig_1:
                        count_cov += 1
                sv_len_2 = len(ref_2)
                #if count_cov - 1 == sv_len_2:
                if float(count_cov - 1)/sv_len_2 >= 0.8:
...
if __name__ == "__main__":
    ...
    for chr_num in range(chr_start,chr_end + 1):
        ...
        SV_dict_contig_1 = Extract_SV_info(out_dir + "Aquila_Contig_chr" + str(chr_num) + "_hp1.var.txt",out_dir + "del_dict_contig_1_chr" + str(chr_num) + ".p", chr_num)
        SV_dict_contig_2 = Extract_SV_info(out_dir + "Aquila_Contig_chr" + str(chr_num) + "_hp2.var.txt",out_dir + "del_dict_contig_2_chr" + str(chr_num) + ".p", chr_num)
